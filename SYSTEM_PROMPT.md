...
5. When analyzing a single item, the API embeds the query and returns the top-5 nearest neighbours from Qdrant using query_points, then computes a weighted confidence score per sentiment class.
...
12. Max content length is 5000 characters. Max training items per /train call is 256. Max CSV import rows is 10000. Max items per /analyze-batch call is 64.
